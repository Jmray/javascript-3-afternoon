...
//Code Here
let evenNumbers  = mixedNumbers.filter((elem) => elem % 2 === 0 );
...
//Code Here
let totalPopulation  = populations.reduce((result, current) => result += current);
...
let orderTotals = orders.map((elem) => elem.price + (elem.price * elem.tax));
...
let bobsTotal = purchases.reduce(function(result, current) { if(current.owner == 'Bob'){return result += current.price;}else{return result;}}, 0)
